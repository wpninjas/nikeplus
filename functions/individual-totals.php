<?php
function nikeplus_individual_totals_defaults() {
    $defaults = array(
        'runner'       => '',
        'show_list'     => false,
        'show_name'     => false,
...
function nikeplus_individual_totals( $args ) {
    $defaults = nikeplus_individual_totals_defaults();
    ...
    if( $totals ) :
        ...
        do_action( 'nikeplus_individual_totals_before', $args );
        if( 'MI' == $opt['distance_unit'] ) {
            $distance = $n->toMiles($totals[0]->distance);
            $pace = $n->calculatePace( $totals[0]->duration, $totals[0]->distance, $toMiles = true );
        } else {
            $distance = $n->toTwoDecimalPlaces( $totals[0]->distance );
            $pace = $n->calculatePace( $totals[0]->duration, $totals[0]->distance, $toMiles = false );
        }
        if( $show_list ) {
            $content .= '<ul id="nikeplus">';

            if( $show_name ) :
                $content .= get_userdata($runner)->display_name;
            endif;
            if( $show_runs ) :
                $content .= '<li id="runs"><span class="runs"></span><span class="nr">'  . $totals[0]->run . ' </span><span class="desc">' . __( 'Total runs', 'nikeplus' ) . '</span></li>';
            endif;
            if( $show_fuel ) :
                $content .= '<li id="fuel"><span class="fuel"></span><span class="nr">'  . number_format( $totals[0]->totalFuel ) . ' </span><span class="desc">' . __( 'Nike fuel', 'nikeplus' ) . '</span></li>';
            endif;
            if( $show_distance ) :
                $content .= '<li id="distance"><span class="distance"></span><span class="nr">' . $distance . '/' . $opt['distance_unit'] . ' </span><span class="desc">' . __( 'Total distance', 'nikeplus' ) . '</span></li>';
            endif;
            if( $show_duration ) :
                $content .= '<li id="time"><span class="time"></span><span class="nr">' . $n->formatDuration($totals[0]->duration) . ' </span><span class="desc">' . __( 'Total time', 'nikeplus' ) . '</span></li>';
            endif;
            if( $show_pace ) :
                $content .= '<li id="pace"><span class="pace"></span><span class="nr">' . $pace . '/' . $opt['distance_unit'] . ' </span><span class="desc">' . __( 'Avg pace', 'nikeplus' ) . '</span></li>';
            endif;
            if( $show_calories ) :
                $content .= '<li id="calories"><span class="calories"></span><span class="nr">' . number_format($totals[0]->calorie ) . ' </span><span class="desc">' . __( 'Total calories', 'nikeplus' ) . '</span></li>';
            endif;
        ...
        /*
            ...
            if( $show_runs ) :
                $content .= '<li>'  . $totals[0]->run . ' ' . __( 'Runs', 'nikeplus' ) . '</li>';
            endif;
            if( $show_fuel ) :
                $content .= '<li>'  . number_format( $totals[0]->totalFuel ) . ' ' . __( 'Fuel', 'nikeplus' ) . '</li>';
            endif;
            if( $show_distance ) :
                $content .= '<li>' . $distance . '/' . $opt['distance_unit'] . ' ' . __( 'Distance', 'nikeplus' ) . '</li>';
            endif;
            if( $show_duration ) :
                $content .= '<li>' . $n->formatDuration($totals[0]->duration) . ' ' . __( 'Duration', 'nikeplus' ) . '</li>';
            endif;
            if( $show_pace ) :
                $content .= '<li>' . $pace . '/' . $opt['distance_unit'] . ' ' . __( 'Pace', 'nikeplus' ) . '</li>';
            endif;
            if( $show_calories ) :
                $content .= '<li>' . number_format($totals[0]->calorie ) . ' ' . __( 'Calories', 'nikeplus' ) . '</li>';
            endif;
        ...
        */
            ...
            $content .= '</tr>';
                ...
                if( $show_runs ) :
                    $content .= '<td>';
                        $content .= $totals[0]->run;
                    $content .= '</td>';
                endif;
                if( $show_fuel ) :
                    $content .= '<td>';
                        $content .= number_format( $totals[0]->totalFuel );
                    $content .= '</td>';
                endif;
                if( $show_distance ) :
                    $content .= '<td>';
                ...
                if( $show_duration ) :
                    $content .= '<td>';
                        $content .= $n->formatDuration($totals[0]->duration);
                    $content .= '</td>';
                endif;
                if( $show_pace ) :
                    $content .= '<td>';
                ...
                if( $show_calories ) :
                    $content .= '<td>';
                        $content .= number_format( $totals[0]->calorie );
                    $content .= '</td>';
                endif;
                $content .= '</tr>';
            $content .= '</table>';
        }
        do_action( 'nikeplus_individual_totals_after' );
        return apply_filters( 'nikeplus_individual_totals', $content );
    endif;

}

function nikeplus_individual_totals_shortcode( $atts ){

    extract( shortcode_atts( nikeplus_individual_totals_defaults(), $atts ) );

    $content = apply_filters( 'nikeplus_individual_totals_shortcode', nikeplus_individual_totals( $atts ) );
    return $content;

}
add_shortcode( 'individual_totals', 'nikeplus_individual_totals_shortcode' );

class nikeplus_individual_totals extends WP_Widget {

    public function __construct() {
        // widget actual processes
        parent::__construct(
            'nikeplus_individual_totals', // Base ID
            __( 'Nike+ Individual Totals', 'nikeplus' ), // Name
            array( 'description' => __( 'List Nike+ Individual Totals', 'nikeplus' ), ) // Args
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $title = apply_filters( 'widget_title', $instance['title'] );
            ...
            echo apply_filters( 'nikeplus_individual_totals_widget', nikeplus_individual_totals( $args ) );
            echo $after_widget;
    }

}
add_action( 'widgets_init', create_function( '', 'register_widget( "nikeplus_individual_totals" );' ) );
