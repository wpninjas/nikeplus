...
class NikePlusPHP {
	...
	public function mostRecentRun() {
		...
		return $this->run(reset($activities)->activityId);
	}
	...
	public function firstRun() {
		...
		return $this->run(end($activities)->activityId);
	}
     ...
     * @return string
     */
    public function toLongHours($time) {
        return intval($time / 3600000);
    }
     ...
     * @return string
     */
    public function formatLongDuration($time, $hideZeroHours = true, $hideSeconds = false) {
        $hours = $this->toLongHours($time);
        $minutes = $this->toMinutes($time);
        $seconds = $this->toSeconds($time);
        $formattedTime = $this->padNumber($minutes);
        if(!$hideSeconds) {
